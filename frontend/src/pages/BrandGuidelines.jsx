...
export default function BrandGuidelines() {
  ...
  const handleDownloadPDF = () => {
    ...
    let y = 60;
    doc.setFontSize(22); doc.text(`${brandName} — Brand Guidelines`, 40, y); y += 30;
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 sm:px-6 py-6 space-y-5 pb-28">
        ...
        {guidelines && !isGenerating && (<>
          {guidelines.logoUsage && (
            <GuidelineSection title="Logo Usage" icon="🖼️" color="purple">
              <ul className="space-y-2">{guidelines.logoUsage.map((r, i) => <li key={i} className="flex gap-2 text-gray-700 text-sm"><span className="text-purple-400 mt-0.5 flex-shrink-0">•</span>{r}</li>)}</ul>
            </GuidelineSection>
          )}
          {guidelines.colorPalette && (
            <GuidelineSection title="Color Palette" icon="🎨" color="blue">
          ...
          {guidelines.typography && (
            <GuidelineSection title="Typography" icon="✍️" color="green">
          ...
          {guidelines.brandVoice && (
            <GuidelineSection title="Brand Voice & Tone" icon="🗣️" color="yellow">
              ...
              {guidelines.brandVoice.guidelines && <ul className="space-y-2 mb-3">{guidelines.brandVoice.guidelines.map((g, i) => <li key={i} className="flex gap-2 text-gray-700 text-sm"><span className="text-yellow-400 flex-shrink-0">•</span>{g}</li>)}</ul>}
          ...
          {guidelines.dosAndDonts && (
            <GuidelineSection title="Do's & Don'ts" icon="✅" color="red">
              <div className="grid sm:grid-cols-2 gap-6">
                <div><p className="font-semibold text-green-700 mb-2 text-sm">✅ Do's</p><ul className="space-y-1">{guidelines.dosAndDonts.dos?.map((d, i) => <li key={i} className="flex gap-2 text-gray-700 text-sm"><span className="text-green-500 flex-shrink-0">✓</span>{d}</li>)}</ul></div>
                <div><p className="font-semibold text-red-700 mb-2 text-sm">❌ Don'ts</p><ul className="space-y-1">{guidelines.dosAndDonts.donts?.map((d, i) => <li key={i} className="flex gap-2 text-gray-700 text-sm"><span className="text-red-500 flex-shrink-0">✗</span>{d}</li>)}</ul></div>
              </div>
            </GuidelineSection>
          )}
          {guidelines.imagery && (
            <GuidelineSection title="Imagery Style" icon="🖼️" color="indigo">
              <ul className="space-y-2">{guidelines.imagery.map((r, i) => <li key={i} className="flex gap-2 text-gray-700 text-sm"><span className="text-indigo-400 flex-shrink-0">•</span>{r}</li>)}</ul>
            </GuidelineSection>
          )}
        </>)}
      </div>
